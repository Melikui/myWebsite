...
Mine.prototype.play = function (ev, obj) {
    var This = this;
    if (ev.which == 1 && obj.className != 'flag') { //后面的条件是为了限制用户标完小红旗后左键就不能点击
        //点击的是左键
...
customize.onclick = function(){
    ...
    var mine = new Mine(parseInt(window.prompt('请输入行数：')),parseInt(window.prompt('请输入列数：')),parseInt(window.prompt('请输入雷数：')));
    mine.init();
}
